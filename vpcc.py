...
"""
 Class to allow for conversions to/from geographic co-ordinate systems
 used in the UK.

 Much of the code here is based on work from the following people and
 URL's.

 Chris Veness 2005-2012.
...
class Point(object):
    """ A single geographic co-ordinate.
    """
    ...
    def __init__(self, scheme, *args):
        """ Init the object with a scheme and position.
        """
        scheme = scheme.lower()
        if not scheme in self.SCHEMES:
            raise InvalidScheme
        self.coords = {}
        if not self._validate_args(scheme, args):
    ...
    def _validate_args(self, scheme, args):
        if scheme == 'gridref':
            # Expect a singe string, but may be supplied as parts
            args = ''.join(args).strip(' ').replace(' ','').upper()
            if re.match("^[A-Z]{2}[0-9]{6}", args):
                self.coords[scheme] = args
                return True
            return False
        # From here on in we need 2 args...
        if len(args) < 2:
            if type(args[0]) is list and len(args[0]) == 2:
                args = args[0]
            else:
                return False

        if scheme == 'osref':
            # Expect 2 numbers, each of at least 3 digits
            for a in args[:2]:
                try:
                    if int(a) < 100:
                        return False
                    ...
                    return False
            self.coords[scheme] = args[:2]
            return True

        #Expect 2 floats...
        if math.fabs(args[0]) > 90.0:
            return False
        if math.fabs(args[1]) > 180.0:
            return False
        self.coords[scheme] = args[:2]
        return True
